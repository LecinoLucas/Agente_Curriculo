  ...
  return (
    <div className="rounded-lg border border-[hsl(var(--border))] bg-white p-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-base font-semibold text-[hsl(var(--text))]">Timeline de eventos</h3>
      </div>
      ...
      {events.length === 0 ? (
        ...
        <div className="mt-4 space-y-3">
          {events.map((event) => (
            <div key={event.id} className="rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--surface-muted))]/20 p-3">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                <div className="text-sm font-medium text-[hsl(var(--text))]">{eventLabel(event.event_type)}</div>
                <div className="text-xs text-[hsl(var(--text-muted))]">{formatDate(event.created_at)}</div>
              </div>
              {event.payload_json ? (
                <pre className="mt-2 max-h-28 overflow-auto rounded-md bg-white/80 p-2 text-xs text-[hsl(var(--text-muted))]">
