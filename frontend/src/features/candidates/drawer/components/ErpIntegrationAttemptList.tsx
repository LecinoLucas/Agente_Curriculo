...
export function ErpIntegrationAttemptList({ attempts }: ErpIntegrationAttemptListProps) {
  if (attempts.length === 0) {
    return <p className="text-sm text-[hsl(var(--text-muted))]">Nenhuma tentativa registrada.</p>;
  }

  return (
    <div className="space-y-2">
      <h4 className="text-sm font-semibold text-[hsl(var(--text))]">Tentativas de Integração</h4>
      <ul className="space-y-2">
        {attempts.map((attempt) => (
          <li key={attempt.id} className="admission-row p-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <span className="text-xs font-medium uppercase tracking-wide text-[hsl(var(--text))]">
