import { ChevronDown, PanelRightClose, PanelRightOpen, RefreshCw, UserPlus, Search, SlidersHorizontal, Plus, Briefcase, MapPin, Award, Layers } from "lucide-react";
...
export function PipelinePage() {
  ...
  // Filter and sort columns based on search and sortOrder
  const mainCols = useMemo(
    () =>
      (board?.columns ?? [])
        .filter((c) => (MAIN_STAGES as ReadonlyArray<string>).includes(c.stage))
        .map((col) => {
          let candidates = col.candidates;
          if (searchQuery.trim()) {
            const query = searchQuery.toLowerCase();
            candidates = candidates.filter((c) =>
              c.candidate_name.toLowerCase().includes(query) ||
              (c.top_skills ?? []).some((s) => s.toLowerCase().includes(query)) ||
              (c.seniority_level ?? "").toLowerCase().includes(query)
            );
          }
          return {
            ...col,
            candidates: sortCandidatesByScore(candidates, sortOrder),
          };
        }),
    [board, sortOrder, searchQuery],
  );
  ...
  // Filter jobs options based on search query
  const filteredJobs = useMemo(() => {
    if (!searchQuery.trim()) return pipelineJobs;
    const query = searchQuery.toLowerCase();
    return pipelineJobs.filter((job) =>
      job.title.toLowerCase().includes(query) ||
      (job.location ?? "").toLowerCase().includes(query)
    );
  }, [pipelineJobs, searchQuery]);
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-6 px-6 py-6 pb-12 text-slate-800">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between border-b border-slate-100 pb-4">
        <div>
          <nav className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-slate-400">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative min-w-[200px] sm:min-w-[260px]">
            ...
            <input
              ...
              className="h-10 w-full rounded-xl border border-slate-200 bg-white pl-9 pr-4 text-xs font-medium text-slate-700 shadow-sm outline-none transition focus:border-[hsl(var(--primary))]/40 focus:ring-2 focus:ring-[hsl(var(--primary))]/5"
            />
          </div>
          ...
          <button
            type="button"
            className="inline-flex h-10 items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 text-xs font-bold text-slate-500 shadow-sm transition hover:bg-slate-50 hover:text-slate-700"
          >
          ...
          {pipelineJobsError ? (
            ...
            <div className="relative">
              <select
                ...
                className="h-10 appearance-none rounded-xl border border-slate-200 bg-white pl-3.5 pr-8 text-xs font-bold text-slate-600 shadow-sm outline-none transition focus:border-[hsl(var(--primary))]/40 focus:ring-2 focus:ring-[hsl(var(--primary))]/5 disabled:opacity-50"
              >
                {pipelineJobsLoading ? (
                  <option value="">Carregando vagas…</option>
                ) : filteredJobs.length === 0 ? (
                  <option value="">Nenhuma vaga correspondente</option>
                ) : (
      ...
      {activeJobId && board && !boardError && (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
            ...
            <div className="mt-2.5 flex items-baseline justify-between">
              ...
              <span className="rounded bg-indigo-50/50 border border-indigo-100 px-1.5 py-0.5 text-[9px] font-bold text-indigo-500">
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
            ...
            <div className="mt-2.5 flex items-baseline justify-between">
              ...
              <span className="rounded bg-amber-50 border border-amber-100 px-1.5 py-0.5 text-[9px] font-bold text-amber-600">
                Agendadas
              </span>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
            ...
            <div className="mt-2.5 flex items-baseline justify-between">
              ...
              <span className="rounded bg-emerald-50 border border-emerald-100 px-1.5 py-0.5 text-[9px] font-bold text-emerald-600">
                Efetivadas
              </span>
            </div>
          </div>
      ...
      {selectedJob && (
        <div className="rounded-2xl border border-slate-100 bg-slate-50/50 p-4 shadow-[0_1px_2px_rgba(0,0,0,0.01)] transition-colors hover:border-slate-200">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-wrap items-center gap-y-2 gap-x-4">
              <div className="flex items-center gap-2">
                <Briefcase className="h-4 w-4 text-slate-400" />
                <span className="text-sm font-bold text-slate-700">{selectedJob.title}</span>
              </div>
          ...
          {selectedJob && !activeJobAcceptsCandidates && !isDraft && (
            <div className="mt-3.5 flex items-center gap-2.5 rounded-xl border border-amber-200 bg-amber-50/50 p-3 text-xs text-amber-800">
      ...
      {activeJobId && (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50/30 px-4 py-2.5 text-xs text-slate-500">
          <div className="flex items-center gap-2 font-semibold">
            <span className={`h-2 w-2 rounded-full ${autoRefreshActive ? "bg-cyan-500 animate-pulse" : "bg-slate-300"}`} />
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 font-bold hover:text-slate-700 disabled:opacity-50"
            >
      ...
      {activeJobId ? (
        isDraft ? (
          ...
          <div className={boardLayoutClass}>
            ...
            {/* Main Board Area */}
            <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.02)] min-w-0">
              ...
              <div className="mb-5 flex flex-col gap-4 border-b border-slate-100 pb-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1 rounded-xl bg-slate-50 border border-slate-150 p-1">
                    <button
                      onClick={() => setSortOrder("score_desc")}
                      className={`rounded-lg px-3 py-1.5 text-[11px] font-bold transition-all ${sortOrder === "score_desc" ? "bg-white text-[hsl(var(--primary))] shadow-[0_1px_2px_rgba(0,0,0,0.05)] border border-slate-100" : "text-slate-400 hover:text-slate-600"}`}
                    >
                      Top Match IA
                    </button>
                    <button
                      onClick={() => setSortOrder("name_az")}
                      className={`rounded-lg px-3 py-1.5 text-[11px] font-bold transition-all ${sortOrder === "name_az" ? "bg-white text-[hsl(var(--primary))] shadow-[0_1px_2px_rgba(0,0,0,0.05)] border border-slate-100" : "text-slate-400 hover:text-slate-600"}`}
                    >
                      Ordem A-Z
                    </button>
                  </div>
                  ...
                  {isBoardRefreshing && (
                    <span className="inline-flex items-center gap-1.5 rounded-lg border border-slate-100 bg-slate-50 px-2.5 py-1.5 text-[10px] font-bold text-slate-500 animate-pulse">
              ...
              {board && !boardError && (
                <div className="overflow-x-auto pb-4">
                  <div className="flex min-w-max items-stretch gap-6 h-[720px] max-h-[85vh]">
                    ...
                    {rejectedCol && (
                      <>
                        <div className="mx-1 w-px self-stretch bg-slate-100 border-r border-dashed border-slate-200" />
                        <KanbanColumn
                          column={rejectedCol}
              ...
              {!showInitialBoardLoading && board && !boardError && totalCandidatos === 0 && (
                <div className="flex flex-col items-center justify-center gap-4 py-16">
                  <div className="max-w-md rounded-2xl border border-slate-150 bg-slate-50/50 p-10 text-center">
                    ...
                    <div className="mt-6 flex flex-col gap-2.5 sm:flex-row sm:justify-center">
                      ...
                      <button
                        ...
                        disabled={!canUse}
                        className="inline-flex h-10 items-center justify-center rounded-xl border border-slate-200 bg-white px-5 text-xs font-bold text-slate-500 shadow-sm transition hover:bg-slate-50 hover:text-slate-700 disabled:opacity-50"
                      >
                        Criar Manualmente
                      </button>
                    </div>
...
function RankingPanel({
  ...
  return (
    <aside
      ...
      className="sticky top-6 flex h-[720px] max-h-[85vh] flex-col rounded-2xl border border-slate-100 bg-white p-5 shadow-[0_4px_12px_rgba(0,0,0,0.03)]"
    >
      <div className="flex items-start justify-between gap-3 border-b border-slate-100 pb-4">
        <div className="min-w-0">
          ...
          <h3 className="mt-1 text-sm font-bold text-slate-800 truncate" title={jobTitle}>{jobTitle}</h3>
        </div>
        <div className="flex shrink-0 items-center gap-1.5">
          {onRefresh && (
            <button
              ...
              onClick={onRefresh}
              disabled={loading}
              className="flex h-8 w-8 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 shadow-sm transition hover:bg-slate-50 disabled:opacity-40"
          ...
          <button
            type="button"
            onClick={onToggle}
            className="flex h-8 w-8 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 shadow-sm transition hover:bg-slate-50"
            title="Fechar"
          >
      ...
      <div id="pipeline-ranking-content" className="mt-4 flex-1 overflow-y-auto ui-scrollbar pr-0.5">
        ...
        {!showInitialLoading && error && (
          <div className="rounded-xl border border-rose-250 bg-rose-50/50 p-4 text-xs font-semibold text-rose-800">
            {error}
          </div>
        )}
        ...
        {!showInitialLoading && !error && ranking && ranking.candidates.length === 0 && (
          <div className="py-12 text-center">
            ...
            <p className="text-xs font-bold text-slate-700">Nenhum ranking disponível</p>
        ...
        {!showInitialLoading && !error && ranking && ranking.candidates.length > 0 && (
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between rounded-xl border border-slate-100 bg-slate-50 px-3.5 py-2 text-[9px] font-black text-slate-400 uppercase tracking-wider">
    ...
    <button
      ...
      className={[
        ...
        hasDealBreakerRejection
          ? "border-rose-200 bg-rose-50/40 hover:border-rose-350 hover:shadow-sm"
          : "border-slate-100 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.01)] hover:border-slate-350 hover:shadow-[0_4px_12px_rgba(0,0,0,0.04)]",
      ].join(" ")}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <p className="truncate text-xs font-bold text-slate-700">{entry.candidate_name}</p>
          </div>
          <div className="mt-1.5 flex items-center gap-2">
            <span className="rounded border border-slate-100 bg-slate-50 px-1.5 py-0.5 text-[9px] font-bold text-slate-400 uppercase tracking-wider">
              {freshnessLabel}
            </span>
          </div>
        </div>
      ...
      {hasDealBreakerRejection && dealBreakerDisplay && (
        <div className="mt-3 rounded-lg border border-rose-150 bg-white p-2.5">
        ...
        <div className="mt-3">
          <div className="flex flex-wrap gap-1">
            {reasonPreview.map((reason) => (
              <span
                key={reason}
                className="inline-flex items-center rounded border border-slate-100 bg-slate-50 px-1.5 py-0.5 text-[9px] font-bold text-slate-500"
              >
                {reason}
              </span>
            ))}
