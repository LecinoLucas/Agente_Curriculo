...
@pytest.mark.asyncio
async def test_bulk_update_unknown_skill_with_fallback_becomes_requirement(
    client: AsyncClient,
    db_session: AsyncSession,
):
    recruiter = await _create_active_user(db_session, "bulk-update-fallback@test.com", "password123", UserRole.RECRUITER)
    headers = await _auth_headers(client, "bulk-update-fallback@test.com", "password123")
    await _create_skill_with_aliases(db_session, "API REST", "api rest", ["rest api"], category="Tecnologia")
    job = await _create_job(
        db_session,
        recruiter.id,
        title="Backend APIs",
        job_area="Tecnologia",
        location="Goiânia API",
            ...
            "jobs": [
                {
                    ...
                    "data": {
                        "skills": [
                            {"name": "APIs REST", "is_mandatory": True, "weight": 9},
                            {"name": "Skill Inexistente", "is_mandatory": False, "weight": 5},
                        ]
                    },
                }
            ],
            "options": {"fallback_unknown_skills_to_requirements": True},
        },
        headers=headers,
    )
    ...
    updated = await db_session.get(JobModel, job.id)
    assert updated is not None
    assert "Skill Inexistente" in (updated.requirements or "")

    rows = await db_session.execute(
        sa.select(SkillModel.name)
        .join(JobRequiredSkillModel, SkillModel.id == JobRequiredSkillModel.skill_id)
        .where(JobRequiredSkillModel.job_id == job.id)
    )
    assert [name for (name,) in rows.all()] == ["API REST"]
